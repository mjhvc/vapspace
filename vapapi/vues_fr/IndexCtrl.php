  ...
  {  
    ...
    elseif (!($this->droits & RESPONSABLE)) {
      ...
      if($this->droits & RESPONSABLE) {
        ...
        $this->vue->teleFile = $this->vue->webm = ''; 
      ...
      if($this->droits & ADMIN) {
        ...
        $this->vue->webm = '<p class="secondPlan">'.WEBMASTER.' <br />'
          .'<a href="'.BASEURL.'index.php?ctrl=index&amp;reset=on&amp;token='.$token.'">'.ENT_CTXT.'</a><br />'
          .'<a href="'.BASEURL.'index.php?ctrl=antenne&amp;action=resetCache&amp;token='.$token.'">'.ENT_CACHANT.' </a></p>'
          .'<br class="nettoyeur" />';       
